...
EM April 8, 2025
    Args:
        ...
        _equipment_: (list[PhEquipment]) Optional list of Ph-Equipment (Process) objects
            which will be included. If None are provided, the Phius Default set will be 
            used: Dishwasher, Clothes Washer, Clothes Dryer, Refrigerator+Freezer
        ...
        _total_lighting_int: (float) The total Interior-Lighting (kWh/a) of the Honeybee-Rooms provided. This total MEL 
            will be evenly divided into the Honeybee-Rooms, then a new Lighting (Process) load 
            created and addded to each Honeybee-Room.

        _total_lighting_ext: (float) The total Exterior-Lighting (kWh/a) of the Honeybee-Rooms provided. This total MEL 
            will be evenly divided into the Honeybee-Rooms, then a new Lighting (Process) load 
            created and addded to each Honeybee-Room.

        _total_lighting_garage: (float) The total Garage-Lighting (kWh/a) of the Honeybee-Rooms provided. This total MEL 
            will be evenly divided into the Honeybee-Rooms, then a new Lighting (Process) load 
            created and addded to each Honeybee-Room.

        _hb_residential_rooms: (list[Room]): The Honeybee Residential Rooms to add the new loads to.

    Returns:

        hb_residential_rooms_: The Honeybee-Rooms with the new Residential electrical
...
ghenv.Component.Name = "HBPH - Set Phius Multi-Family Residential Room Loads"
DEV = honeybee_ph_rhino._component_info_.set_component_params(ghenv, dev=False)
if DEV:
    from honeybee_ph_rhino.gh_compo_io.program import set_phius_mf_res as gh_compo_io
    reload(gh_compo_io)
    reload(gh_io)
...
#-------------------------------------------------------------------------------
gh_compo_interface = gh_compo_io.GHCompo_SetPhiusMFResidentialRoomLoads(
    IGH,
    _equipment_,
    _total_mel,
    _total_lighting_int,
    ...
    _hb_residential_rooms,
)

hb_residential_rooms_ = gh_compo_interface.run()