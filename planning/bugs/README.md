...
|------|-----------|--------|-------|-----|
| Create Elec Equip writes the string "None" into numeric equipment fields | `HBPH - Create PH Equipment` | **Merged to main 2026-09-15** ([#95](https://github.com/PH-Tools/honeybee_grasshopper_ph/pull/95)) — `_normalize_input_value` in the worker (unset: None, empty text, "None"; numeric attributes coerced to their type; bad text raises). CPython stub check passes. Remaining: canvas check in Rhino, then archive | [#94](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/94) | [`create-elec-equip-none-string.md`](create-elec-equip-none-string.md) |
