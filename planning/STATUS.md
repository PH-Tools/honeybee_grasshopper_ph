...
|------|------|--------|---------|
| Already-exported models carry inflated window-type tables | Field remediation + PHX hardening | **Scoped, remediation underway** (2026-08-26) — full-Dropbox scan: only 2310 affected (3 hbjson, 19 WUFI XML, 1 mwp; Aug 11-12 only). Detector script + quarantine marker done; clean exports already exist, so no re-export needed. Remaining: Phase 4 PHX stale-row guard, and Ed confirming which .mwp went to Phius for Round 5 | [`bugs/exported-models-inflated-window-types.md`](bugs/exported-models-inflated-window-types.md) |
