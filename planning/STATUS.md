...
| Create Elec Equip: document the `quantity` input | Docs (cross-repo follow-up) | **Merged** (2026-09-15, PR [#99](https://github.com/PH-Tools/honeybee_grasshopper_ph/pull/99)) — the input description now states the default (1) and that `energy_demand` is per-unit. The pin half of the issue was **not** hand-edited: `requirements.txt` is owned by the release orchestrator, which picks up `honeybee-ph` 1.33.67 / `PHX` 1.57.1 on the next release run. Remaining: that release, plus a canvas check that an existing component's tooltip picks up the new text | [#98](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/98) | issue-only (spec in #98) |
