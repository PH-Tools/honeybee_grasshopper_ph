...
|------|------|--------|-------|---------|
| PH Additional Zone: PHPP v10 reduction-factor inputs | Feature (upstream `honeybee_ph` #122; export in `PHX` #65) | **Merged to main** (2026-09-12, PR #85) — five optional factor inputs; `.ghuser` rebuilt. Remaining: release. The four load/cooling factors have no PHPP/WUFI effect until `PHX` #65 | [#78](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/78) | issue-only (spec in #78) |
...
| Remove deprecated 'Add PH Equipment' + 'Phius MF Res Calculator' | Refactor (upstream `honeybee_ph` #79 step 1 released 1.33.65) | **Implemented on branch** `refactor/remove-deprecated-equipment-compos-84` — both dead components, their workers, registry entries, and the MF calc `.ghuser` deleted; `_deprecated_/` package gone; PR #87 open. Remaining: canvas Update-GHCompos check, merge, release notes naming replacements | [#84](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/84) | issue-only (spec in #84) |
