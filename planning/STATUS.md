...
|------|------|--------|---------|
| Already-exported models carry inflated window-type tables | Field remediation + PHX hardening | **Implemented on branch `fix/bug59-field-remediation`** (2026-08-26) — scoped (only 2310: 23 artifacts, Aug 11-12), detector script + quarantine marker + doc corrections done; PHX stale-row guard implemented on `fix/phpp-components-stale-rows` ([PHX #99](https://github.com/PH-Tools/PHX/issues/99)). Open: Ed confirms which .mwp went to Phius for Round 5; merge both branches; then archive | [`bugs/exported-models-inflated-window-types.md`](bugs/exported-models-inflated-window-types.md) |
