...
|------|------|--------|-------|---------|
| PH Additional Zone: PHPP v10 reduction-factor inputs | Feature (upstream `honeybee_ph` #122; export in `PHX` #65) | **Implemented on branch** `feat/addnl-zone-reduction-factors-78` — worker + wrapper source done, scratch-checked against the issue's criteria; `.ghuser` rebuilt with the five input nodes and regenerated (wrapper byte-identical). Remaining: PR review, merge, release | [#78](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/78) | issue-only (spec in #78) |
