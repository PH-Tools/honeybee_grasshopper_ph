...
|------|------|--------|-------|---------|
| PH Additional Zone: PHPP v10 reduction-factor inputs | Feature (upstream `honeybee_ph` #122; export in `PHX` #65) | **Released** (2026-09-12, v1.39.0; PR #85) — five optional factor inputs; `.ghuser` rebuilt. The four load/cooling factors have no PHPP/WUFI effect until `PHX` #65 | [#78](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/78) | issue-only (spec in #78) |
...
| Bldg Segment `mechanical_cooling_` input | Feature (cross-repo, part 3 of `honeybee_ph` #110; after `PHX` PR #113; contributor PR) | **Released** (2026-09-12, v1.40.0; PR #77) — input wired through wrapper and worker; blank/None normalizes to False; `.ghuser` regenerated | [PR #77](https://github.com/PH-Tools/honeybee_grasshopper_ph/pull/77) | issue-only (upstream [`honeybee_ph` #110](https://github.com/PH-Tools/honeybee_ph/issues/110)) |
| Create Elec Equip writes the string "None" into numeric equipment fields | Bug fix | **Merged to main** (2026-09-15, PR #95; issue closed) — `_normalize_input_value` in `set_object_attributes`; CPython stub check passes. Remaining: canvas check in Rhino (and ideally a reproduction of how the text reached the float-hinted input), then release | [#94](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/94) | [`bugs/create-elec-equip-none-string.md`](bugs/create-elec-equip-none-string.md) |
