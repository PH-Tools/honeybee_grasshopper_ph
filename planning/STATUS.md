...
| Create PH Equipment ignores a boolean `False` input (`in_conditioned_space`) | Bug fix | **In review** (2026-09-15, branch `fix/create-elec-equip-bool-inputs`) — `_normalize_input_value` applies boolean attributes by value; CPython stub check passes. Remaining: canvas re-check (`False` → `false` in the HBJSON), then release. Found during the #94 canvas check | [#96](https://github.com/PH-Tools/honeybee_grasshopper_ph/issues/96) | issue-only (evidence in #96) |
